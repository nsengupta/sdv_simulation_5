...
use crate::observation_records::diagnostic::{elapsed_since_session, DiagnosticRecord};
...
async fn given_twin_boot_when_both_sinks_wired_then_ledger_and_diagnostic_share_session_start(
    ...
    assert!(
        ...
        "expected at least init and state diagnostics"
    );

    let session = ledger_rows[0].session_start_unix_nanos;
    assert!(
        ledger_rows.iter().all(|r| r.session_start_unix_nanos == session),
        "ledger rows must share one session anchor"
    );
    assert!(
        diagnostics
            .iter()
            .all(|d| d.session_start_unix_nanos == session),
    ...
    for d in &diagnostics {
        assert_eq!(
            ...
            elapsed_since_session(d.recorded_at_unix, d.session_start_unix_nanos),
        );
    }

    for row in &ledger_rows {
        assert_eq!(
            elapsed_since_session(row.recorded_at_unix, row.session_start_unix_nanos),
            row.recorded_at_unix.saturating_sub(Duration::from_nanos(session as u64)),
        );
    }
}

#[tokio::test]
async fn given_twin_boot_when_observing_streams_then_recorded_at_unix_monotonic_within_each_stream() {
    let (diag_tx, mut diag_rx) = mpsc::unbounded_channel();
    let (trans_tx, mut trans_rx) = mpsc::channel(16);
    ...
    for pair in ledger_rows.windows(2) {
        assert!(
            pair[1].recorded_at_unix >= pair[0].recorded_at_unix,
            "ledger recorded_at_unix must be non-decreasing"
        );
    }
    for pair in diagnostics.windows(2) {
        assert!(
            pair[1].recorded_at_unix >= pair[0].recorded_at_unix,
            "diagnostic recorded_at_unix must be non-decreasing"
        );
    }
}
...
async fn given_twin_with_transition_sink_only_when_boot_then_ledger_carries_session_pair() {
    ...
    assert!(row.session_start_unix_nanos > 0);
    assert!(row.recorded_at_unix >= Duration::from_nanos(row.session_start_unix_nanos as u64));
}
